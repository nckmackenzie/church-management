    ...
    public function GetAccounts($type)
    {
        if($type == 1){
            $this->db->query('SELECT ID,UCASE(accountType) AS accountType FROM tblaccounttypes 
                              WHERE ((accountTypeId = :expense) OR (accountTypeId = :asset)) AND (deleted=0) AND (isBank = 0) AND (parentId <> 0) AND (active = 1) ORDER BY accountType');
        }else{
            $this->db->query('SELECT ID,UCASE(accountType) AS accountType FROM tblaccounttypes 
                              WHERE ((accountTypeId = :expense) OR (accountTypeId = :asset)) AND (deleted=0) AND (isBank = 0) AND (parentId <> 0) AND (forGroup = 1) AND (active = 1) ORDER BY accountType');
        }
